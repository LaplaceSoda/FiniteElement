...
for i = 1:m
    ...
    [l,u] = lu(( a0 * M + a1 * C + K ));
    V(:,i+1) = u\(l\( M * ( a0 * V(:,i)+ a2* dV(:,i) + a3 * a(:,i)) + ...
               C * ( a1 * V(:,i)+ a4* dV(:,i) + a5 * a(:,i)) + double(R(ti))));
    a(:,i+1) = a0 * ( V(:,i+1) - V(:,i) ) - a2 * dV(:,i) - a3 * a(:,i);
    dV(:,i+1) = dV(:,i) + a6 * a(:,i) + a7 * a(:,i+1);
end
